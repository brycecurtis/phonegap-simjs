...
public class FileUtils extends Plugin {
		...
		try {
			try {
				if (action.equals("testSaveLocationExists")) {
					...
					JSONObject obj = getFile(args.getString(0), args.getString(1), args.getJSONObject(2), true);
					return new PluginResult(status, obj, "window.localFileSystem._castEntry");
				}
				else if (action.equals("getFile")) {
					JSONObject obj = getFile(args.getString(0), args.getString(1), args.getJSONObject(2), false);
					return new PluginResult(status, obj, "window.localFileSystem._castEntry");
				}
	...
	private boolean atRootDirectory(String filePath) {
		if (filePath.equals(Environment.getExternalStorageDirectory().getAbsolutePath() + "/filesystem/temp") ||
				filePath.equals(Environment.getExternalStorageDirectory().getAbsolutePath() + "/filesystem/persistent")) {
			return true;
		}
		return false;
	}
	...
	private JSONObject requestFileSystem(int type) throws IOException, JSONException {
		...
		if (type == TEMPORARY) {
			...
			if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
				...
				// Create the persistent dir if it doesn't exist.
				File fp = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + 
					"/filesystem/persistent");
				fp.mkdirs();
			} else {
