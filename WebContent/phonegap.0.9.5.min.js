...
var sim = false;
var phonegapDir = '';
console.log("base="+document.URL);
var lists = document.getElementsByTagName("script");
for (var i = 0; i < lists.length; i++) {
  console.log("script tag="+lists[i].src);
  var src = ""+lists[i].src;
  var p = src.indexOf("phonegap.0.9.5.min.js");
  if (p > 0) {
	  phonegapDir = src.substring(0, p);
	  break;
  }
}
console.log("PhoneGap is loaded from "+phonegapDir);
...
if (sim) {
    window.setTimeout(
		function() {
			try {
				console.log("Ready to fire onNativeReady");
				PhoneGap.JSCallbackHost = "localhost";
				PhoneGap.onNativeReady.fire();
				console.log("Fired onNativeReady");
			}
			catch (e) {
...
var simulator = {
		...
		platform: "Android",
		name: "mydevice",
		phonegapVersion: "0.9.5"
};


simulator.prompt = function(s1, s2) {
	console.log("simulator.prompt("+s1+","+s2+")");
    // polling = PhoneGap.callNative("usePolling", "gap_callbackServer:");
    if ((s1 == "usePolling") && (s2 == "gap_callbackServer:")) {
        return "false";
    }
    // PhoneGap.callNative("restartServer", "gap_callbackServer:");
    if ((s1 == "restartServer") && (s2 == "gap_callbackServer:")) {
        return null;
    }
    // PhoneGap.JSCallbackPort = PhoneGap.callNative("getPort", "gap_callbackServer:");
    if ((s1 == "getPort") && (s2 == "gap_callbackServer:")) {
        return "8080";
    }
    // PhoneGap.JSCallbackToken = PhoneGap.callNative("getToken", "gap_callbackServer:");
    if ((s1 == "getToken") && (s2 == "gap_callbackServer:")) {
        return "phonegap-java/PhoneGap?s1=getJavascript&callbackId="+PhoneGap.callbackIdPrefix+":";
    }
    // msg = PhoneGap.callNative("", "gap_poll:");
    if ((s1 == "") && (s2 == "gap_poll:")) {
        return "";
    }
    
    // var r = PhoneGap.callNative(this.stringify(args), "gap:"+this.stringify([service, action, callbackId, true]));
    var xhReq = new XMLHttpRequest();
    xhReq.open("GET", "/phonegap-java/PhoneGap?s1="+s1+"&s2="+s2, false);
...
/*
simulator.poll = function() {
	return null;
}
simulator.exec = function(args, service, action, callbackId) {
	console.log("Simulator.exec("+args+","+service+","+action+","+callbackId+")");
	
	if (service == "Accelerometer") {
		if (action == "getAcceleration") {
			var date = new Date();
			var accel = {
					x: Math.floor(Math.random()*1000)/100.0,
					y: Math.floor(Math.random()*1000)/100.0,
					z: Math.floor(Math.random()*1000)/100.0,
					timestamp: date.getTime()
			};
			return PhoneGap.stringify({message: accel, status: PhoneGap.callbackStatus.OK});
		}
	}
	if (service == "Device") {
		if (action == "getDeviceInfo") {
			var r = {};
			var message = {};
			message.uuid = simulator.uuid;
			message.version = simulator.version;
			message.platform = simulator.platform;
			message.name = simulator.name;
			message.phonegap = simulator.phonegapVersion;
			r.message = message;
			r.status = PhoneGap.callbackStatus.OK;
			return PhoneGap.stringify(r);
		}
	}
	if (service == "Camera") {
		if (action == "takePicture") {
			return PhoneGap.stringify({message: "../ibm.jpg", status: PhoneGap.callbackStatus.OK});			
		}
	}
	if (service == "Compass") {
		if (action == "getHeading") {
			var heading = Math.floor(Math.random()*361);
			return PhoneGap.stringify({message: heading, status: PhoneGap.callbackStatus.OK});
		}

	}
	if (service == "Notification") {
		if (action == "beep") {
			alert("Beep "+args[0]+" times.");
		}
			...
			confirm(args[0], args[1], args[2]);
		}
		else if (action == "prompt") {
			console.log("Notification.prompt()");
		}
		else if (action == "activityStart") {
			console.log("Notification.activityStart()");			
		}
		else if (action == "activityStop") {
			console.log("Notification.activityStop()");			
		}
		else if (action == "progressStart") {
			console.log("Notification.progressStart()");			
		}
		else if (action == "progressValue") {
			console.log("Notification.progressValue()");			
		}
		else if (action == "progressStop") {
			console.log("Notification.progressStop()");			
		}
		return PhoneGap.stringify({message: "", status: PhoneGap.callbackStatus.OK});

	}
	if (service == "Network Status") {
		if (action == "isReachable") {
			return PhoneGap.stringify({message: NetworkStatus.REACHABLE_VIA_WIFI_NETWORK, status: PhoneGap.callbackStatus.OK});			
		}
	}
	return null;
}
...
PhoneGap.Channel.join(function() {
    ...
    setTimeout(function() {
            if (PhoneGap.UsePolling) {
            	...
            	var polling = PhoneGap.callNative("usePolling", "gap_callbackServer:");
...
PhoneGap.callbackIdPrefix = PhoneGap.createUUID();
PhoneGap.callbacks = {};
...
PhoneGap.callNative = function(s1, s2) {
    if (sim) {
        return simulator.prompt(s1, s2);
    }
    else {
        return prompt(s1, s2);
    }
...
PhoneGap.exec = function(success, fail, service, action, args) {
    try {
        var callbackId = PhoneGap.callbackIdPrefix + ":" + service + PhoneGap.callbackId++;
        if (success || fail) {
...
PhoneGap.JSCallbackHost = "127.0.0.1";
...
PhoneGap.JSCallback = function() {
    ...
    //console.log("XHR GET=http://"+PhoneGap.JSCallbackHost+":"+PhoneGap.JSCallbackPort+"/"+PhoneGap.JSCallbackToken);
    xmlhttp.open("GET", "http://"+PhoneGap.JSCallbackHost+":"+PhoneGap.JSCallbackPort+"/"+PhoneGap.JSCallbackToken , true);
    xmlhttp.send();
};
...
PhoneGap.JSCallbackPolling = function() {
    ...
    var msg = PhoneGap.callNative("", "gap_poll:");
    if (msg) {
        setTimeout(function() {
            try {
                var t = eval(""+msg);
...
Geolocation.prototype.watchPosition = function(successCallback, errorCallback, options) {
    ...
    console.log("Geolocation.watchPosition("+id+")");
...
Geolocation.prototype.clearWatch = function(id) {
    console.log("Geolocation.clearWatch("+id+")");
